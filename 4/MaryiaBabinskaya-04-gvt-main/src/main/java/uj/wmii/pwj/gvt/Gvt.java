...
enum COMMANDS{
            ...
            try {
                ...
                for (; version >= 0 && n >= 0; n--, version--) {
                    try (Scanner sc = new Scanner(new File(".gvt/v" + version + "/comment"))) {
                        res += version + ": " + sc.nextLine() + "\n";
                    }
                }
                exitHandler.exit(0, res);
    ...
    version{
        public void action(ExitHandler exitHandler,  Gvt gvt, String... args) {
            int version = 0;
            try {
                version = args.length < 2 ? gvt.lastVersion() : Integer.parseInt(args[1]);
...
public class Gvt {
    ...
    boolean zeroArgsControl(String...args) {
        if(args.length == 0) {
    ...
    void commandNameControl(String s) {
        boolean exists = true;
        try {
            COMMANDS.valueOf(s);
